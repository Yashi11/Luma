...
const configureLocalServicePorts = async (): Promise<void> => {
  ...
  selected.add(tutorPort);
  const requestedVisualCopilotPort = requestedPort(
    process.env.VISUAL_COPILOT_PORT,
    8082,
  );
  const visualCopilotPort = await findAvailablePort(
    requestedVisualCopilotPort,
    selected,
  );

  process.env.SENSING_PORT = String(sensingPort);
  process.env.TUTOR_PORT = String(tutorPort);
  process.env.VISUAL_COPILOT_PORT = String(visualCopilotPort);
...
ipcMain.on('onboarding-complete', (_event, profile: object) => {
  ...
  if (!VISUAL_COPILOT_MODE) startObserver();
...
app
  ...
  .then(async () => {
    ...
    selectionController = new SelectionController(
      Number(process.env.VISUAL_COPILOT_PORT || '8082'),
